...
function doProvAndCityRelation() {
    ...
    $.each(cityJson,
        function (i, val) {
            if (val.item_code.substr(0, 2) == $("#ToState").val().substr(0, 2) && val.item_code.substr(2, 4) != '0000' && val.item_code.substr(4, 2) == '00') {
                sb.append("<option value='" + val.item_code + "'>" + val.item_name + "</option>");
            }
        });
...
function doCountyAndAddressRelation() {
    var countyVal = $("#ToArea").val();
    if (countyVal == "-1") {
        $("#areaSelectShowId").text("");
        $("#ToAddress").prop("style", "");
    } else {
        var county_code = $("#ToArea").val();
        var state_code = countyVal.substr(0, 2) + "0000";
        var city_code = countyVal.substr(0, 4) + "00";
        var state_name = "";
        var city_name = "";
        var county_name = "";
        var sb = new StringBuffer();
        $.each(cityJson,
            function (i, val) {
                if (val.item_code == state_code) {
                    state_name = val.item_name;
                }
                if (val.item_code == city_code) {
                    city_name = val.item_name;
                }
                if (val.item_code == county_code) {
                    county_name = val.item_name;
                }

            });
        sb.append(state_name).append(city_name).append(county_name);
        $("#areaSelectShowId").text(sb.toString())
        $("#areaSelectShowId").attr("data-areacode", county_code);
        var width = $("#areaSelectShowId").width();
        var addressWidth = (parseInt(width) + parseInt("15")).toString();
        $("#ToAddress").prop("style", "padding-left:" + addressWidth + "px");
    }
}
...
function receiverAddrAddOrEditOpenWin(addressid,Title) {
    if (addressid != 0) {
        $.ajax({
                type:"post",
                url:"/myReceiver/findReceiverByRid?rid="+addressid,
                dataType: 'json',
                success: function (data) {
                    $("#ToName").val(data.rname);
                    $("#ToMobile").val(data.rphone);
                    // $("#ToState").val(data.provinces);
                    // $("#ToCity").text(data.city);
                    // $("#ToArea").val(data.county);
                    $("#ToState").children().each(function () {
                        var pro_code= $(this).val();
                        var pro_name= $(this).text();
                        if(pro_name==data.provinces){
                    ...
                    $("#chooseCity").after(sb1.toString());
                    $("#ToCity").children().each(function () {
                        var city_code= $(this).val();
                        var city_name= $(this).text();
                        if(city_name==data.city){
                            $(this).prop("selected","selected")
                        }
                    });
                    var cityVal = $("#ToCity").val();
                    ...
                    $.each(cityJson,
                        function (i, val) {
                            if (cityVal == '110100' || cityVal == "120100" || cityVal == "310100" || cityVal == "500100") {
                                if (val.item_code.substr(0, 3) == cityVal.substr(0, 3) && val.item_code.substr(4, 2) != '00') {
                                    sb2.append("<option value='" + val.item_code + "'>" + val.item_name + "</option>");
                                }
                            } else {
                                if (val.item_code.substr(0, 4) == cityVal.substr(0, 4) && val.item_code.substr(4, 2) != '00') {
                                    sb2.append("<option value='" + val.item_code + "'>" + val.item_name + "</option>");
                    ...
                    $("#chooseCounty").after(sb2.toString());
                    $("#ToArea").children().each(function () {
                        var county_code= $(this).val();
                        var county_name= $(this).text();
                        if(county_name==data.county){
                            $(this).prop("selected","selected")
                            $("#areaSelectShowId").attr("data-areacode", county_code);
                        }
                    });
                    $("#areaSelectShowId").text(data.provinces+data.city+data.county)
                    var width = $("#areaSelectShowId").width();
                    var addressWidth = (parseInt(width) + parseInt("15")).toString();
                    $("#ToAddress").prop("style", "padding-left:" + addressWidth + "px");
                    $("#ToAddress").val(data.addr);
                    $("#AddressID").val(data.rid);
    ...
    $("#addressOperTitle").text(Title);
    $("#addr_layer").attr("style", "display: block;")
}
function openLayer() {
    $("#addr_layer").attr("style", "display: block;")
}